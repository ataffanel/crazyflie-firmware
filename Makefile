...
# Each target is in a different line, so they are executed one after the other even when the processor has multiple cores (when the -j option for the make command is > 1). See: https://www.gnu.org/software/make/manual/html_node/Parallel.html
	@$(MAKE) -f $(CRAZYFLIE_BASE)Makefile --no-print-directory clean_version
	@$(MAKE) -f $(CRAZYFLIE_BASE)Makefile --no-print-directory compile
	@$(MAKE) -f $(CRAZYFLIE_BASE)Makefile --no-print-directory print_version
	@$(MAKE) -f $(CRAZYFLIE_BASE)Makefile --no-print-directory size
